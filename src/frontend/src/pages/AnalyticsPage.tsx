...
import { ChevronDown, ChevronUp, FileText, CreditCard, AlertCircle, Receipt, Settings2, Share2, ArrowUpDown } from 'lucide-react';
import { useGetAllProjects, useGetAllBills, useGetAllPayments, useGetAllClients } from '../hooks/useQueries';
...
export default function AnalyticsPage() {
  ...
  const totalPayments = projectMetrics.reduce((sum, m) => sum + m.totalPayments, 0);
  const totalOutstanding = projectMetrics.reduce((sum, m) => sum + (m.outstanding > 0 ? m.outstanding : 0), 0);
  const totalGst = projectMetrics.reduce((sum, m) => sum + (m.outstanding > 0 ? m.gst : 0), 0);
  ...
  const totalOutstandingForChart = sortedOutstandingData.reduce((sum, item) => sum + item.outstanding, 0);
  const maxOutstanding = sortedOutstandingData.length > 0 ? sortedOutstandingData[0].outstanding : 0;
  ...
  const handleShareChart = async () => {
    ...
    try {
      ...
      // Draw bars
      let yOffset = 160;
      sortedOutstandingData.forEach((item) => {
        const barWidth = maxOutstanding > 0 ? ((item.outstanding / maxOutstanding) * 800) : 0;
        ...
        // Bar gradient
        const gradient = ctx.createLinearGradient(250, yOffset - 20, 250 + actualBarWidth, yOffset - 20);
        ...
        ctx.fillStyle = '#FFFFFF';
        ctx.font = 'bold 14px "Century Gothic", Arial, sans-serif';
        ctx.fillText(formatCurrency(item.outstanding), 250 + actualBarWidth + 10, yOffset);
      ...
      canvas.toBlob(async (blob) => {
        ...
        const file = new File([blob], 'outstanding-chart.png', { type: 'image/png' });
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                {(selectedProjects.length > 0 || selectedClient !== 'all' || selectedYear !== 'all' || selectedFinancialYear !== 'all' || startDate || endDate || sortBy !== 'outstanding') && (
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div>
                  <Label className="text-xs font-normal text-[#555555] mb-1.5 block">Projects</Label>
                  <div className="h-9">
                    <Popover>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                ...
                <div>
                  <Label className="text-xs font-normal text-[#555555] mb-1.5 block">Sort By</Label>
                  <Select value={sortBy} onValueChange={(value) => setSortBy(value as 'outstanding' | 'name')}>
                    <SelectTrigger className="bg-white hover:bg-[#F5F5F5] border-[#BDBDBD] h-9 text-xs font-normal">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="shadow-lg bg-gradient-to-br from-orange-50 to-orange-100 border-orange-200 rounded-xl hover:shadow-xl transition-all duration-300">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-normal text-[#555555]">Total Bills</CardTitle>
        ...
        <Card className="shadow-lg bg-gradient-to-br from-green-50 to-green-100 border-green-200 rounded-xl hover:shadow-xl transition-all duration-300">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-normal text-[#555555]">Total Payments</CardTitle>
        ...
        <Card className="shadow-lg bg-gradient-to-br from-pink-50 to-pink-100 border-pink-200 rounded-xl hover:shadow-xl transition-all duration-300">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-normal text-[#555555]">Total Outstanding</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs text-[#555555] mt-1 font-normal">Positive outstanding only</p>
          </CardContent>
        </Card>
        ...
        <Card className="shadow-lg bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200 rounded-xl hover:shadow-xl transition-all duration-300">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-normal text-[#555555]">Total GST (18%)</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs text-[#555555] mt-1 font-normal">On positive outstanding</p>
          </CardContent>
        </Card>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {projectMetrics.map(({ project, totalBills, totalPayments, outstanding, gst }) => {
          const isOverpaid = outstanding < 0;
          const displayOutstanding = outstanding > 0 ? outstanding : Math.abs(outstanding);
          
          return (
            <Card key={project.id} className="shadow-lg hover:shadow-xl transition-all duration-300 bg-white rounded-xl overflow-hidden">
              <div className="bg-gradient-to-r from-blue-100 to-purple-100 px-6 py-4 border-b border-blue-200">
                <CardTitle className="text-xl font-bold text-[#0078D7]">{project.name}</CardTitle>
                <p className="text-sm text-[#555555] mt-1 font-normal">{project.client}</p>
              </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-gradient-to-br from-orange-50 to-orange-100 p-4 rounded-lg border border-orange-200 hover:shadow-md transition-shadow">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs font-bold text-[#555555]">Bills</span>
                    </div>
                    <div className="text-xl font-bold text-[#FFA500]">{formatCurrency(totalBills)}</div>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-green-50 to-green-100 p-4 rounded-lg border border-green-200 hover:shadow-md transition-shadow">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs font-bold text-[#555555]">Payments</span>
                    </div>
                    <div className="text-xl font-bold text-[#28A745]">{formatCurrency(totalPayments)}</div>
                  </div>

                  <div className={`${isOverpaid ? 'bg-gradient-to-br from-green-50 to-green-100 border-green-200' : 'bg-gradient-to-br from-pink-50 to-pink-100 border-pink-200'} p-4 rounded-lg border hover:shadow-md transition-shadow`}>
                    <div className="flex items-center gap-2 mb-2">
                      <AlertCircle className={`h-5 w-5 ${isOverpaid ? 'text-[#28A745]' : 'text-[#E91E63]'}`} />
                      <span className="text-xs font-bold text-[#555555]">Outstanding</span>
                    </div>
                    <div className={`text-xl font-bold ${isOverpaid ? 'text-[#28A745]' : 'text-[#E91E63]'}`}>
                      {isOverpaid ? '-' : ''}{formatCurrency(displayOutstanding)}
                    </div>
                  ...
                  <div className="bg-gradient-to-br from-purple-50 to-purple-100 p-4 rounded-lg border border-purple-200 hover:shadow-md transition-shadow">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs font-bold text-[#555555]">GST (18%)</span>
                    </div>
      ...
      {projectMetrics.length === 0 && !isLoading && (
        <Card className="shadow-md bg-white rounded-xl">
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-xl font-bold text-[#555555] mb-2">No Projects Found</h3>
            <p className="text-[#757575] font-normal">Try adjusting your filters to see more results.</p>
          </CardContent>
        </Card>
      )}
        ...
        <Card className="shadow-lg bg-[#2D2D2D] border-[#4A90A4] rounded-xl overflow-hidden">
          <div ref={chartRef} className="p-6">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-2xl font-bold text-white mb-2">
                  Details of Outstanding Amounts – <span className="text-[#FF6B6B]">{getTodayDate()}</span>
                </h2>
            ...
            <div className="space-y-3">
              ...
              {sortedOutstandingData.map((item, index) => {
                const barWidth = maxOutstanding > 0 ? (item.outstanding / maxOutstanding) * 100 : 0;
                
                return (
                  <div key={index} className="flex items-center gap-4">
                    <div className="w-48 text-white text-sm font-normal truncate" title={item.projectName}>
                      {item.projectName}
                    </div>
                    <div className="flex-1 relative">
                      <div className="h-10 bg-gradient-to-r from-[#FF8C42] to-[#FFA500] rounded-md relative overflow-hidden shadow-lg" style={{ width: `${barWidth}%`, minWidth: '120px' }}>
