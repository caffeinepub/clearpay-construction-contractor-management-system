...
import { AlertCircle, CreditCard, FileText, Receipt } from "lucide-react";
...
export default function ReportsPage() {
  ...
  const filteredBills = bills.filter((b) => {
    if (selectedProjects.length > 0 && !selectedProjects.includes(b.projectId))
      return false;
    return true;
  });

  const filteredPayments = payments.filter((p) => {
    if (selectedProjects.length > 0 && !selectedProjects.includes(p.projectId))
      return false;
    return true;
  });
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="shadow-sm bg-gradient-to-br from-blue-50 to-blue-100 border-blue-200">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-[#555555]">
              Total Bills
            </CardTitle>
